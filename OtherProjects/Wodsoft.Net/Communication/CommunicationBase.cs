...
namespace Wodsoft.Net.Communication
{
    public class CommunicationBase : ICommunication, IDisposable
    {
        ...
        private void socket_ReceiveCompleted(object sender, SocketEventArgs e)
        {
            byte[] data;
            Guid id;
            CommunicationReceiveEventArgs receiveEventArgs;
            switch (e.Data[0])
            {
                case 3:
                    if (e.DataLength < 21)
                    ...
                    if (PreviewReceive != null)
                        PreviewReceive(this, receiveEventArgs);
                    data = new byte[18];
                    data[0] = 4;
                    for (int i = 1; i < 17; i++)
                        data[i] = e.Data[i];
                ...
                case 5:
                    ...
                    for (int i = 0; i < e.DataLength - 21; i++)
                        receiveEventArgs.Data[offset + i] = e.Data[21 + i];
        ...
        {
            get { return Socket.IsConnected && !_Disposed; }
        }
